...
#ifdef DDE_LINUX
/*
 * i386 code maps IO ports to addresses in range [0x10000, 0x40000]. However we
 * cannot ensure this in DDE, and thus we cannot distinguish between IO ports
 * and memory this way. Therefore we map IO ports to a memory region where no
 * real user memory is mapped.
 */
